...
const int NUM_STEPS = 10;    // Full 100-step simulation for trajectory matching
...
public:
    ...
    void initialize_agents() {
        ...
        for (int i = 0; i < NUM_AGENTS && i < NUM_INIT_AGENTS; i++) {  // Use dynamic limit from generated data
            agents[i].x = AGENT_INIT_DATA[i][0];
            agents[i].y = AGENT_INIT_DATA[i][1];
            agents[i].angle = AGENT_INIT_DATA[i][2];
        }
        std::cout << "[TB] Agents initialized from pre-computed data (circle, 40% radius, pointing inward)" << std::endl;
        std::cout << "[TB] Initialized " << std::min(NUM_AGENTS, NUM_INIT_AGENTS) << " agents (requested: " << NUM_AGENTS << ", available: " << NUM_INIT_AGENTS << ")" << std::endl;
    }
    ...
    void write_agents_to_rtl() {
        ...
        for (int i = 0; i < NUM_AGENTS && i < NUM_INIT_AGENTS; i++) {
            // Write X coordinate
